...
def parse_from_file_obj(fp):
    ...
    Returns:
        Instance of MailParserEx with raw email parsed
    """
    return MailParserEx.from_file_obj(fp)
...
def parse_from_file(fp):
    ...
    Returns:
        Instance of MailParserEx with raw email parsed
    """
    return MailParserEx.from_file(fp)
...
def parse_from_file_msg(fp):
    ...
    Returns:
        Instance of MailParserEx with raw email parsed
    """
    return MailParserEx.from_file_msg(fp)
...
def parse_from_string(s):
    ...
    Returns:
        Instance of MailParserEx with raw email parsed
    """
    return MailParserEx.from_string(s)
...
def parse_from_bytes(bt):
    ...
    Returns:
        Instance of MailParserEx with raw email parsed
    """
    return MailParserEx.from_bytes(bt)


class MailParserEx(object):
    """
    MailParserEx package provides a standard parser that understands
    most email document structures like official email package.
    MailParserEx handles the encoding of email and split the raw email for you.
    ...
    def from_bytes(cls, bt):
        ...
        if six.PY2:
            raise MailParserExEnvironmentError(
            ...
            if not p.is_multipart():
                ...
                if filename:
                    ...
                    if ';' in content_disposition:
                        content_disposition=content_disposition.split(';', 1)[0]
