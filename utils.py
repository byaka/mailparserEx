...
from .exceptions import MailParserExOSError, MailParserExReceivedParsingError
...
def msgconvert(email):
    ...
    try:
        ...
        raise MailParserExOSError(message)
...
def parse_received(received):
    ...
    Raises:
        MailParserExReceivedParsingError -- Raised when a
    ...
    for pattern in RECEIVED_COMPILED_LIST:
        ...
        if len(matches) == 0:
            ...
            msg = "More than one match found for %s in %s" % (
                pattern.pattern, received)
            log.error(msg)
            raise MailParserExReceivedParsingError(msg)
        else:
...
def receiveds_parsing(receiveds):
    ...
    for idx, received in enumerate(receiveds):
        ...
        except MailParserExReceivedParsingError:
